...
const content: Record<TourStep, StepContent> = {
  ...
  workspace: { target: ['.home-intro select'], title: "这次在哪个文件夹里工作？", body: "点这里选一个项目文件夹，或添加电脑上的其他目录。你让 AI 读文件、写文章或改代码时，它会从这个位置开始。", tip: "只是聊天也要先选一个文件夹。已有会话保留原路径；换项目时新建会话再选。" },
  ...
  attachments: { target: ['button[aria-label="添加内容"]'], title: "把需要的材料一起告诉它", body: "点 + 可以引用本地文件。你也可以在输入框里粘贴完整文件路径，或粘贴截图。附上材料，再说明希望 AI 怎样处理。", tip: "图片能否直接读取，取决于当前模型通道的能力；页面会提示。输入 @ 可找文件，输入 / 可查看快捷命令。" },
  skills: { target: ['[data-guide="skills"]'], title: "Skills：可复用的做事方法", body: "点这里选择已经准备好的工作说明，例如写作规范。它们会随本轮消息交给 AI。第一次使用可以先不选。", tip: "选择 Skill 不等于已经执行。仍需写下任务并发送。" },
