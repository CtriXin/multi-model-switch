...
export function ChannelModels({
    ...
    try {
      ...
      for (const item of result.proposals)
        for (const field of item.fields)
          picks[fieldKey(item.model, field)] = proposedByDefault(field);
  ...
  return (
    <section className="channel-models" aria-label="通道模型管理">
      ...
      {refresh && (
        <Dialog title="最新能力对比" size="wide" close={() => setRefresh(undefined)}>
          <div className="capability-review">
            ...
            <footer>
              ...
              <button className="button" onClick={() => setRefresh(undefined)}>
                取消
              </button>
              <button
                className="button primary"
