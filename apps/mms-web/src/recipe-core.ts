...
export function parseRecipe(raw: string): Recipe {
  ...
  if (typeof value.format !== "string" || !["mms-work-recipe-v1", "mms-work-recipe-v2"].includes(value.format)) throw new Error("不支持这个任务模板版本。");
  ...
  if (Object.keys(requirements).some(key => !["image", "reasoning"].includes(key))) throw new Error("模板含有当前版本不认识的模型能力要求，请升级 Pilot 后再导入。");
  if (value.interpolation !== undefined && value.interpolation !== "none") throw new Error("不支持模板的变量替换方式。");
  const literal = legacy || value.interpolation === "none";
  const recipe: Recipe = {
    format: "mms-work-recipe-v2",
    ...
    ...(literal ? { interpolation: "none" as const } : {}),
  ...
  if (literal && recipe.variables.length) throw new Error("纯文本模板不能声明变量。");
  if (!literal) {
...
export function scrubSharedText(input: string): { text: string; removed: number } {
  ...
  replace(/\b(?:api[_ -]?key|access[_ -]?token|refresh[_ -]?token|client[_ -]?secret|secret|password|credential|authorization)["']?\s*[:=]\s*(?:"(?:\\.|[^"\\\n])*"|'(?:\\.|[^'\\\n])*'|[^\s,;&]+)/gi, "[已移除凭据赋值]");
...
export function prepareExport(value: Recipe): { recipe: Recipe; json: string; filename: string; removed: number } {
  ...
  const safe = {
    ...
    variables: value.interpolation === "none" ? [] : variableNames(prompt, input, output),
    ...(value.interpolation === "none" ? { interpolation: "none" as const } : {}),
  };
  ...
  parseRecipe(json); // Validate the final downloadable bytes, including whitespace.
  return { recipe, json, filename, removed };
}
