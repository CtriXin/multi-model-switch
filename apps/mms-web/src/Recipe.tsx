  ...
  return <div className="recipe-import">
    ...
    {recipe && <Dialog title={`导入「${recipe.title}」`} close={() => { generation.current++; setRecipe(undefined); }}>
      <div className="recipe-dialog">
        ...
        {!!recipe.variables.length && <div className="recipe-variables">{recipe.variables.map(name => <label key={name}>{name}<input aria-label={`变量 ${name}`} value={values[name] || ""} onChange={e => { setValues({ ...values, [name]: e.target.value }); setShowPrompt(false); }} placeholder="填写本次使用的值" /></label>)}</div>}
        ...
        <button type="button" disabled={!!variableError} onClick={() => {
...
export function RecipeExport({ detail, close }: { detail: SessionDetail; close: () => void }) {
  ...
  return <Dialog title="保存为任务模板" close={close}>
    <div className="recipe-dialog">
      ...
      {preview && <section className="recipe-reviewed"><p role="status">已检查，移除 {preview.removed} 处识别到的内容。以下是最终文件：</p><pre className="recipe-preview">{preview.json}</pre><button type="button" onClick={download}>确认并下载模板</button></section>}
    </div>
  </Dialog>;
}
