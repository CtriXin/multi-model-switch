...
function fixture({ failSave = false, preference = "" } = {}) {
  ...
  const actual = new Set(["App", "ModelExplorer", "SettingsPage", "Models", "ChannelModels", "TaskSettings", "connection-state"]);
  function load(name) {
    if (modules.has(name)) return modules.get(name).exports;
    const mod = { exports: {} }; modules.set(name, mod);
    const filename = path.join(appDir, "src", name + (name === "connection-state" ? ".ts" : ".tsx"));
  ...
  function effortSelect() {
    const menu = mount(taskSettings().type, taskSettings().props).tree;
    const contents = menu.props.children(noop, true);
    const effort = nodes(contents).find(n => n.type === load("ModelExplorer").EffortSelect);
    return nodes(effort.type(effort.props)).find(n => n.type === "select");
  }
  ...
  return { root, calls, flush, editAndSave, effortSelect, taskSettings, find,
...
test("saving shared GLM effort refreshes the actual new-task default without reload", async () => {
  ...
  try {
    await f.flush();
    assert.match(text(f.effortSelect()), /MMS 默认 · xhigh/);
    await f.editAndSave();
    assert.match(text(f.effortSelect()), /MMS 默认 · high/);
    assert.equal(f.effortSelect().props.value, "");
...
test("shared save refreshes facts while preserving explicit task and browser effort", async () => {
  for (const kind of ["task", "browser"]) {
    ...
    try {
      ...
      await f.editAndSave();
      assert.match(text(f.effortSelect()), /MMS 默认 · high/);
      assert.equal(f.effortSelect().props.value, "xhigh");
