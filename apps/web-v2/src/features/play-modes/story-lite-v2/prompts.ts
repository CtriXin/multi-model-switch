...
export function buildStoryLiteV2SeededScene(seedLabel: string, sceneId: string): StoryLiteV2Scene {
  ...
  if (sceneId === 'secure-shield') {
    return {
      ...
      responses: buildSeededResponses([
        { role: 'guide', modelName: 'Model Beta', text: '别松手。你既然先押主线，就要把最直接的失控点彻底压住。', tone: '推进' },
        { role: 'partner', modelName: 'Model Alpha', text: '你现在稳住的是盘面，不是人心。真正会在事后留下裂痕的部分，已经开始发生了。', tone: '提醒' },
        { role: 'variable', modelName: 'Model Gamma', text: '最奇怪的是，异常点并没有因为你押主线就退场，它像是在故意等你回头看它。', tone: '逼近' },
      ]),
  ...
  if (sceneId === 'save-mother') {
    return {
      ...
      responses: buildSeededResponses([
        { role: 'guide', modelName: 'Model Beta', text: '你先救了人的这一边，现在得立刻想办法把主线损耗追回来。', tone: '补救' },
        { role: 'partner', modelName: 'Model Alpha', text: '至少这一刻，你没有把最重要的人丢给“等事情忙完再说”。这不是小事。', tone: '撑住' },
        { role: 'variable', modelName: 'Model Gamma', text: '异常像是专门配合你的选择被点亮的。它不是背景噪音，更像有人在引导你站队。', tone: '引导' },
      ]),
      choices: buildSeededChoices(sceneId),
    }
  }
  ...
  if (sceneId === 'trace-signal') {
    return {
      ...
      responses: buildSeededResponses([
        { role: 'guide', modelName: 'Model Beta', text: '真相重要，但它不能代替止损。你必须尽快决定这些新线索该服务哪条主线。', tone: '收束' },
        { role: 'partner', modelName: 'Model Alpha', text: '你在追真相的时候，别忘了真正会疼的人和关系还留在现实那一边。', tone: '牵引' },
        { role: 'variable', modelName: 'Model Gamma', text: '异常线已经回应你了。问题不是“有没有第三条路”，而是你敢不敢继续赌。', tone: '诱发' },
      ]),
      choices: buildSeededChoices(sceneId),
    }
  }
  ...
  return {
    ...
    responses: buildSeededResponses([
      { role: 'guide', modelName: 'Model Beta', text: '先处理最会立刻失控的那条主线。越晚止损，后面的每一步都会更贵。', tone: '决断' },
      { role: 'partner', modelName: 'Model Alpha', text: '别把你最在乎的人或关系放到“等忙完再说”的位置，那通常就是来不及。', tone: '拉扯' },
      { role: 'variable', modelName: 'Model Gamma', text: '题面被设计得太像二选一了。真正该看的，是那个不该这么整齐出现的异常。', tone: '诡异' },
    ]),
