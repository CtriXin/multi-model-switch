...
const topCardStyle = computed(() => {
  ...
  const opacity = Math.abs(dragX.value) / 200
  return {
  ...
  <div class="flex flex-col h-full space-y-6 overflow-hidden">
    ...
    <div class="shrink-0 space-y-3">
      <div class="flex items-center justify-between px-2">
        ...
        <button @click="emit('refresh')" class="p-2 rounded-full hover:bg-white/5 active:rotate-180 transition-all duration-700">
      ...
      <div class="flex gap-2.5 overflow-x-auto no-scrollbar px-2 py-2">
        <button
          ...
          :class="categories.includes(cat)
            ...
            : 'bg-white/5 border-white/5 grayscale opacity-40 hover:grayscale-0'"
        >
          ...
          <span class="text-[9px] font-black tracking-widest leading-none">{{ categoryMeta[cat].label }}</span>
        </button>
      </div>
    </div>
    ...
    <div class="flex-1 relative mt-4 flex items-center justify-center">
      ...
      <!-- 加载状态 -->
      <div v-if="loading && !visibleCandidates.length" class="flex flex-col items-center gap-4 animate-pulse">
        <div class="w-16 h-16 rounded-[32px] border-2 border-dashed border-accent/30 animate-spin"></div>
        <span class="text-[10px] font-black uppercase tracking-[0.4em] text-text-tertiary">Syncing Arena...</span>
      </div>

      <div v-else-if="!visibleCandidates.length" class="text-center p-10 glass-v3 rounded-[40px] border-dashed border-white/10">
         ...
         <button @click="emit('refresh')" class="mt-6 px-6 py-3 bg-accent text-white rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl">重新载入</button>
      </div>
      ...
      <div v-else class="relative w-full h-full max-h-[460px]">
        <div
          v-if="loading"
          class="absolute top-4 left-1/2 z-20 -translate-x-1/2 rounded-full border border-accent/20 bg-white/80 px-3 py-1 text-[9px] font-black uppercase tracking-[0.28em] text-accent shadow-lg backdrop-blur dark:bg-[#111827]/70"
        >
          Syncing
        </div>
        <div
          ...
          >
            ...
            <div class="flex items-center justify-between relative z-10">
              <div class="flex items-center gap-3">
                <div class="w-12 h-12 rounded-2xl bg-black/5 flex items-center justify-center text-2xl">
              ...
              <div class="px-3 py-1 rounded-full bg-black/5 border border-black/5">
                <span class="text-[10px] font-black text-text-secondary uppercase tracking-widest">
                  {{ topic.difficulty === 'casual' ? '日常挑战' : topic.difficulty === 'deep' ? '深度模式' : '灵魂哲思' }}
                </span>
              </div>
            </div>
            ...
            <div class="flex-1 flex flex-col justify-center py-8 relative z-10">
               <div class="mb-4">
                  <Zap :size="24" stroke-width="3" class="text-accent opacity-20" />
               </div>
            ...
            <!-- 装饰纹理 -->
            <div class="absolute -bottom-10 -right-10 w-40 h-40 bg-accent/5 blur-[60px] rounded-full"></div>
          </div>
        </div>
      </div>
    </div>
...
.animate-v3-gradient {
  ...
  animation: move-gradient 2.5s linear infinite;
}
