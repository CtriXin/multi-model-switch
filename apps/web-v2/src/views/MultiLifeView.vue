...
// INTERACTION LOCK: Check if all models in the current round have finished speaking
        ...
        <transition name="ios-swap" mode="out-in">
          <div :key="phase" class="flex-1 flex flex-col overflow-hidden relative">
            ...
            <div v-if="phase === 'setup' && !caseData" class="flex-1 flex flex-col items-center justify-center gap-8 px-6">
              ...
              <div class="text-center space-y-2"><h2 class="text-3xl font-black text-text-primary uppercase tracking-tight">多重人生</h2><p class="text-sm text-text-tertiary opacity-60 max-w-xs mx-auto">三名证人，三种真相。击碎矛盾，还原起始。</p></div>
            ...
            <div v-else-if="phase === 'setup' && caseData" class="flex-1 flex flex-col overflow-hidden relative">
              <div class="flex-1 overflow-y-auto custom-scrollbar px-6 pt-10 pb-60">
                <div class="max-w-2xl mx-auto space-y-10">
                  <div class="text-center space-y-6">
                    <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-accent/10 text-accent text-[10px] font-black uppercase tracking-widest">Case Briefing</div>
                    <h2 class="text-3xl font-black text-text-primary uppercase tracking-tight">{{ caseData.title }}</h2>
                    <div class="p-8 rounded-[40px] bg-accent/5 border border-accent/10 text-base text-text-primary leading-loose shadow-inner min-h-[10em] text-left relative">
                  ...
                  <div v-if="introDone" class="space-y-4 animate-in fade-in duration-1000">
                    <div class="flex items-center gap-3 px-4 mb-4 opacity-40">
                      <div class="h-px flex-1 bg-text-primary"></div>
                      <span class="text-[9px] font-black uppercase tracking-[0.2em]">待核实身份</span>
                      <div class="h-px flex-1 bg-text-primary"></div>
                    </div>
                      ...
                      <div v-for="(role, idx) in caseData.roles" :key="role.id" class="p-5 rounded-[32px] border border-white/5 bg-white shadow-sm flex items-center justify-between" :class="ROLE_STYLES[idx].border">
                        <div class="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            <p v-else class="text-[10px] text-text-quaternary italic uppercase tracking-widest">等待分配资料</p>
                          </div>
                        </div>
              ...
              <div v-if="introDone" class="absolute bottom-0 left-0 right-0 p-6 bg-white/90 dark:bg-white/[0.05] backdrop-blur-2xl border-t border-white/10 animate-in slide-in-from-bottom-full duration-500">
                <div class="max-w-md mx-auto space-y-3">
                  <button @click="store.pickRandomModels()" class="w-full h-14 rounded-2xl border-2 border-accent/20 bg-accent/5 text-accent font-black tracking-widest text-[11px] uppercase active:scale-95 transition-all flex items-center justify-center gap-3">
                    <RefreshCw :size="16" stroke-width="3.5" :class="processing ? 'animate-spin' : ''" /> 重新分配角色代理
                  </button>
                  <button @click="handleBegin" class="w-full h-14 rounded-2xl bg-accent text-white font-black uppercase tracking-widest text-[11px] active:scale-95 shadow-xl shadow-accent/20 flex items-center justify-center gap-3 transition-all lab-breathing-btn">
                    <Fingerprint :size="18" stroke-width="4" /> 开始实地调查
                  </button>
                </div>
              </div>
            </div>
            ...
            <!-- Phase: Investigation -->
            <div v-else-if="phase === 'investigation' && started" class="flex-1 flex flex-col overflow-hidden">
              <div class="flex-1 overflow-y-auto custom-scrollbar px-4 sm:px-8 py-6 space-y-10 scroll-smooth pb-60">
                <div v-for="r in [...rounds].reverse()" :key="r.id" class="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
                  <div class="p-6 rounded-[32px] bg-accent/5 border border-accent/10 space-y-4 shadow-sm relative">
                    <div class="flex items-center justify-between">
                      <span class="text-[10px] font-black text-text-tertiary uppercase tracking-[0.2em]">回合 R{{ r.roundNumber }} · 现场还原</span>
                    ...
                    <div v-for="(resp, idx) in r.responses" :key="resp.roleId" class="group relative perspective-1000">
                      <div class="relative w-full transition-all duration-700 preserve-3d" :class="isFlipped(r.id, resp.roleId) ? 'rotate-y-180' : ''">
                        ...
                        <div v-if="resp.challengeResponse || streamingTexts[`${r.id}-ch-${resp.roleId}`]" class="absolute inset-0 backface-hidden rotate-y-180 p-6 rounded-[32px] border border-amber-500/30 bg-amber-50 shadow-xl flex flex-col h-full overflow-hidden">
                          <div class="flex items-center justify-between mb-4 shrink-0">
                            <div class="flex items-center gap-2 text-amber-600 font-black"><CheckCircle2 :size="16" /><span class="text-[10px] uppercase tracking-widest">补充证词</span></div>
              ...
              <!-- Fixed Action Area with Interaction Lock -->
              <div v-if="lastRound && !lastRound.playerChoice" 
                class="shrink-0 p-4 sm:p-6 border-t border-white/10 bg-white/90 backdrop-blur-2xl absolute bottom-0 left-0 right-0 shadow-[0_-20px_40px_rgba(0,0,0,0.05)] transition-all duration-500"
                :class="allResponsesDone ? 'translate-y-0 opacity-100' : 'translate-y-full opacity-0 pointer-events-none'"
              >
                <div class="max-w-3xl mx-auto space-y-4">
                  <button @click="handleAccept" class="w-full h-14 sm:h-16 rounded-2xl bg-accent text-white shadow-xl active:scale-95 transition-all flex items-center justify-center gap-3 lab-breathing-btn font-black uppercase tracking-[0.2em] text-sm">信任当前结论 · 进入下一轮</button>
                  <div class="flex items-center gap-2 sm:gap-3">
                    <button v-for="resp in lastRound.responses" :key="'ch-' + resp.roleId" @click="handleChallenge(resp.roleId)" class="flex-1 h-12 rounded-xl bg-white border-2 border-black/5 text-amber-500 active:scale-95 transition-all overflow-hidden flex flex-col items-center justify-center"><span class="text-[7px] font-black uppercase tracking-widest opacity-40">质疑角色</span><span class="text-[10px] font-black truncate w-full text-center px-1 uppercase">{{ caseData?.roles.find(r => r.id === resp.roleId)?.name }}</span></button>
                  </div>
                </div>
              </div>

              <!-- Next Round Button (Already Locked by Processing) -->
              <div v-else-if="lastRound && lastRound.playerChoice && phase !== 'resolution'" class="shrink-0 p-4 sm:p-6 border-t border-white/10 bg-white/90 absolute bottom-0 left-0 right-0">
                <button @click="handleNextRound" :disabled="processing" class="w-full h-14 rounded-2xl bg-text-primary text-surface-1 font-black uppercase tracking-widest text-[11px] active:scale-95 disabled:opacity-40 flex items-center justify-center gap-3 shadow-xl"><span v-if="processing" class="flex items-center gap-2"><Loader2 :size="16" class="animate-spin" /> 正在分析破绽...</span><span v-else>推演下一幕现场</span></button>
              </div>
            </div>
            ...
            <div v-else-if="ending" class="flex-1 overflow-y-auto custom-scrollbar px-6 py-10 animate-in zoom-in-95 duration-700">
              <div class="max-w-4xl mx-auto space-y-10 text-center">
                ...
                <h2 class="text-4xl font-black text-text-primary uppercase tracking-tight">调查终局报告</h2>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 text-left">
                  <div class="p-8 rounded-[40px] bg-cyan-50 border border-cyan-400/10 space-y-4 shadow-sm"><span class="text-[10px] font-black text-cyan-600 uppercase tracking-widest">你的叙事推演</span><p class="text-sm leading-loose text-text-primary whitespace-pre-wrap">{{ ending.playerNarrative }}</p></div>
                  <div class="p-8 rounded-[40px] bg-emerald-50 border border-emerald-400/10 space-y-4 shadow-sm"><span class="text-[10px] font-black text-emerald-600 uppercase tracking-widest">最终真相定案</span><p class="text-sm leading-loose text-text-primary whitespace-pre-wrap">{{ ending.truthNarrative }}</p></div>
                </div>
