...
<template>
  <div class="flex flex-col h-full overflow-hidden bg-surface-0">
    ...
    <main class="flex-1 relative overflow-hidden flex flex-col px-3 sm:px-6 pb-3 sm:pb-6">
      <div class="w-full max-w-4xl mx-auto flex-1 flex flex-col glass-v3 rounded-[32px] lg:rounded-[48px] shadow-2xl border border-white/10 overflow-hidden bg-white/95 dark:bg-white/[0.03] relative z-10">
        ...
        <transition name="ios-swap" mode="out-in">
          <div :key="store.phase" class="flex-1 flex flex-col overflow-hidden">
              ...
              <div class="max-w-2xl mx-auto space-y-8">
                <template v-if="!store.currentPuzzle">
                  ...
                  <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <button v-for="puzzle in store.puzzles" :key="puzzle.id" @click="store.selectPuzzle(puzzle)" class="group text-left p-6 rounded-[32px] bg-white border-2 border-black/5 hover:border-accent/30 transition-all duration-500 shadow-sm active:scale-95">
                      <div class="flex items-center justify-between mb-4">
                        <span class="px-2 py-0.5 rounded-lg bg-accent/10 text-[8px] font-black uppercase tracking-widest text-accent">{{ CATEGORY_LABELS[puzzle.category] }}</span>
                ...
                <template v-else>
                  <div class="text-center space-y-10 py-10 animate-in fade-in duration-700">
                    ...
                    <div class="max-w-xs mx-auto space-y-6">
                      <select v-model="modelSelectValue" class="w-full h-14 px-6 rounded-2xl bg-white border-2 border-black/5 text-sm font-black text-text-primary outline-none focus:border-accent/40 transition-all appearance-none text-center">
                        <option :value="null">推荐：智能均衡模式</option>
                        <option v-for="m in store.availableModels" :key="m.id" :value="m.id">{{ m.name }}</option>
                      </select>
                      <button @click="store.startGame()" class="w-full h-16 rounded-3xl bg-accent text-white font-black uppercase tracking-widest text-xs active:scale-95 lab-breathing-btn shadow-2xl">开始解密</button>
                    </div>
                  </div>
                </template>
              </div>
            ...
            <div v-else-if="store.phase === 'playing'" class="flex-1 flex flex-col overflow-hidden relative">
              <div class="px-6 py-4 border-b border-black/5 bg-accent/5 backdrop-blur-md">
              ...
              <div ref="chatRef" class="flex-1 overflow-y-auto custom-scrollbar px-6 py-8 space-y-8 pb-32">
                <div v-for="(q, idx) in store.questions" :key="idx" class="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                  ...
                  <div class="flex justify-start">
                    <div class="max-w-[85%] px-6 py-4 rounded-[28px] rounded-bl-md bg-white border-2 border-black/5 shadow-sm">
                      <div v-if="q.status === 'pending'" class="flex items-center gap-2 py-1 text-xs text-text-tertiary">
                      ...
                      <template v-else>
                        <p class="text-base leading-loose font-medium">
                          ...
                          <span v-if="answerBody(q.answer)" class="text-text-primary lab-flowing-text whitespace-pre-wrap">{{ answerBody(q.answer) }}</span>
                        </p>
                      </template>
                    </div>
                  </div>
              ...
              <!-- STANDARDIZED LAB ACTION POD -->
              <div class="absolute bottom-0 left-0 right-0 p-4 sm:p-6 bg-white/80 backdrop-blur-xl border-t border-black/5">
                <div class="relative max-w-3xl mx-auto group">
                  <div class="absolute -inset-1 bg-gradient-to-r from-accent/20 to-indigo-500/20 rounded-[36px] blur opacity-0 group-focus-within:opacity-100 transition duration-500"></div>
                  <div class="relative flex items-center bg-white border-2 border-black/5 rounded-[32px] p-1.5 shadow-xl transition-all group-focus-within:border-accent/40">
                    <input 
                      v-model="inputText" 
                      @keydown="handleKeydown" 
                      :disabled="store.processing" 
                      type="text" 
                      :placeholder="store.processing ? '主持人回答中…' : '向主持人提问细节...'" 
                      class="flex-1 h-12 px-6 bg-transparent outline-none text-sm font-black placeholder:text-text-quaternary" 
                    />
                    <button 
                      @click="submitQuestion" 
                      :disabled="!inputText.trim() || store.processing" 
                      class="w-12 h-12 rounded-2xl bg-accent text-white flex items-center justify-center shadow-lg active:scale-95 disabled:opacity-20 transition-all lab-breathing-btn"
                    >
            ...
            <div v-else-if="store.phase === 'completed' || store.phase === 'abandoned'" class="flex-1 overflow-y-auto custom-scrollbar px-6 py-10 text-center">
              <div class="max-w-xl mx-auto space-y-10 animate-in zoom-in-95 duration-700">
                ...
                <div class="p-10 rounded-[48px] bg-accent/5 border border-accent/10 relative">
...
@keyframes breathing { 0% { box-shadow: 0 0 0 0 rgba(var(--color-accent-rgb, 110, 89, 255), 0.4); } 70% { box-shadow: 0 0 0 12px rgba(var(--color-accent-rgb, 110, 89, 255), 0); } 100% { box-shadow: 0 0 0 0 rgba(var(--color-accent-rgb, 110, 89, 255), 0); } }
...
@keyframes iosIn { from { opacity: 0; transform: translateX(40px) scale(0.98); filter: blur(15px); } to { opacity: 1; transform: translateX(0) scale(1); filter: blur(0); } }
@keyframes iosOut { from { opacity: 1; transform: translateX(0); } to { opacity: 0; transform: translateX(-40px) scale(0.98); filter: blur(15px); } }
