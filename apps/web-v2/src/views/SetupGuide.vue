...
import { 
  ...
  Settings, Package, Shield, ExternalLink, Key, Menu, ArrowRight, ChevronLeft
...
const coreTiles = [
  ...
  { 
    ...
    stats: () => `${appStore.models.length} 可选基因`
  }
]
...
<template>
  <div class="flex flex-col h-full overflow-hidden bg-transparent">
    <!-- Group 1: Floating Capsule Header (FIXED: Menu on Left) -->
          ...
          <button @click="showFullGuide = !showFullGuide" 
            ...
            <div class="flex items-center gap-4">
              <div v-if="configuredCount > 0" class="hidden sm:flex items-center gap-2 px-4 py-2 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-500 text-[10px] font-black uppercase tracking-widest">
                <Check :size="12" stroke-width="4" /> {{ configuredCount }} 厂商就绪
              </div>
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mt-8">
                <div class="space-y-4">
                  ...
                  <p class="text-sm text-text-secondary leading-relaxed">
                    无需任何 API Key，立即在左侧菜单点击“对话模式”即可体验完整的交互流程。
                  </p>
                </div>
                <div class="space-y-4">
                  ...
                  <p class="text-sm text-text-secondary leading-relaxed">
                    前往 <button @click="router.push('/settings')" class="text-accent underline font-bold underline-offset-4">设置中心</button> 填入你的厂商密钥。
                  </p>
                </div>
              </div>
            </div>
