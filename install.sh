...
print_dry_run_plan() {
    ...
    echo "  $(t "只删 MMS 自己写的条目，不卸载第三方程序，不动你自己改过的文件" "Only entries MMS wrote itself are removed; no third-party program is uninstalled and nothing you edited is touched")"
...
for port in range(start, start + limit):
    try:
        ...
        try:
            ...
            pass
    if str(response.getheader("Server") or "").startswith("MMSWeb"):
        print(port)
        break
PY
}
...
start_mms_web_detached() {
    ...
    if wait_for_mms_web "$port"; then
        ...
        echo "  $(t "停止:" "Stop it with:") pkill -f 'mms-web --config-root'"
        return 0
    fi
...
remove_retired_skill_entry() {
    ...
    if [ -L "$target" ]; then
        ...
        case "$link_target" in
            *"/vendor/$vendor_suffix"|*"/vendor/$vendor_suffix/")
                rm -f "$target"
                _note_retired_removal "$target"
                ;;
            *)
    ...
    [ -d "$target" ] || return 0
    if [ -f "$target/SKILL.md" ] && grep -Fq "$marker" "$target/SKILL.md" 2>/dev/null; then
        rm -rf "$target"
        _note_retired_removal "$target"
        return 0
    fi
...
# Drop hook registrations and MCP servers that belong to the retired packs.
# The file is backed up before any write, and unrelated entries are preserved.
prune_retired_claude_settings() {
    local settings_path="$REAL_HOME/.claude/settings.json"
    local py_output=""

    [ -f "$settings_path" ] || return 0

    py_output="$("$(_python_bin)" - "$settings_path" <<'PY'
import json
import shutil
import sys
from datetime import datetime
...
HOOK_FRAGMENTS = (
    ...
    "/.claude/hooks/token-monitor-hook.sh",
    "/.claude/hooks/claude-context-restore-hint.sh",
    "claude-map-auto-index.sh",
    "claude-codegraph-auto-index.sh",
)
MCP_KEYS = ("brainkeeper", "mindkeeper", "codegraph")
...
def retired_command(command):
    text = str(command or "")
    return any(fragment in text for fragment in HOOK_FRAGMENTS)
    ...
    for event, groups in list(hooks.items()):
        ...
        for group in groups:
            ...
            for entry in entries:
                ...
                if retired_command(command):
                    removed.append("hooks.%s: %s" % (event, command))
                    continue
...
if isinstance(mcp_servers, dict):
    for key in MCP_KEYS:
        if key in mcp_servers:
            ...
            removed.append("mcpServers.%s" % key)
    if not mcp_servers:
...
backup = settings_path.with_suffix(
    ".json.mms-retired-backup.%s" % datetime.now().strftime("%Y%m%d%H%M%S")
)
...
PY
    ...
    while IFS= read -r line; do
        case "$line" in
            backup:*)
                echo "  $(t "已备份 Claude 全局设置" "Backed up Claude global settings"): ${line#backup:}"
                ;;
            removed:*)
                _note_retired_removal "~/.claude/settings.json ${line#removed:}"
                ;;
        esac
    done <<< "$py_output"
...
cleanup_retired_optional_packs() {
    ...
    # Claude hook copies installed for the retired packs
    for hook_file in rtk-rewrite.sh token-monitor-hook.sh claude-context-restore-hint.sh; do
        if [ -f "$hook_dir/$hook_file" ]; then
            rm -f "$hook_dir/$hook_file"
            _note_retired_removal "$hook_dir/$hook_file"
        fi
    done
    ...
    # ops-env-safe path map lives in the MMS config dir and is inert without the skill
    remove_retired_marked_file "$REAL_HOME/.config/mms/ops-env-safe.toml" \
        "Managed by MMS optional ops-env-safe pack"
