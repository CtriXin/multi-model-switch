...
if [ -x "$BIN_DIR/mms" ]; then
    ...
    if [ "$PREVIEW_CHANNEL_INSTALL" -eq 0 ] && [ "$RUN_SETUP" -eq 1 ] && { [ ! -f "$active_config_path_value" ] || [ ! -f "$active_credentials_path_value" ]; }; then
        echo "$(t "检测到首次使用，启动配置向导..." "First-time setup detected, launching setup wizard...")"
        echo ""
        "$BIN_DIR/mms" || true
        DID_LAUNCH=1
    elif [ "$PREVIEW_CHANNEL_INSTALL" -eq 0 ] && { [ ! -f "$active_config_path_value" ] || [ ! -f "$active_credentials_path_value" ]; }; then
