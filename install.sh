...
for package_dir in mms_opencode mms_codex mms_claude mms_agy mms_pi; do
    copy_dir_safely "$SOURCE_DIR/$package_dir" "$MMS_HOME/$package_dir" "$package_dir package" "$package_dir package"
done
for stale_module in "$MMS_HOME"/mms_opencode_*.py "$MMS_HOME"/mms_codex_*.py "$MMS_HOME"/mms_claude_*.py "$MMS_HOME"/mms_agy_*.py "$MMS_HOME"/mms_pi_*.py; do
