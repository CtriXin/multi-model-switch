...
from mms_core import _preference_runtime_overlay, load_user_preferences
...
#
# MMS ships one built copy of @ctrixin/pi-btw and injects it into every Pi it
# starts, so a side question never disturbs the main task in Pilot and in the
# terminal alike. Everything below is read-only: Pi's own config trees are
# inspected to decide whether to stay out of the way, never written.

_PI_BTW_PACKAGE_MARKER = "pi-btw"
_PI_BTW_SOURCE_NAME = "SOURCE.json"
_PI_BTW_BUNDLE_NAME = "index.ts"
_PI_BTW_BANNER = "// @generated by scripts/sync_pi_btw.py"
_PI_BTW_TRUSTED_BYTES = 4_000_000
# (path, mtime_ns, size) -> verified. Rehashing a few hundred KB on every launch
# would be free, but this also keeps the warning to one pass per build.
_PI_BTW_VERIFIED: set[tuple[str, int, int]] = set()
...
def _pi_btw_integrity_key(bundle):
    try:
        stat = bundle.stat()
    except OSError:
        return None
    return str(bundle), stat.st_mtime_ns, stat.st_size


def _pi_btw_bundle_verified(bundle, vendor, log):
    """Refuse to inject a file that is not what the last sync recorded.

    The bundle is a generated artifact that runs with full system access in
    every MMS Pi session, so a missing record, a drifted sha256 or a size jump
    all fail closed: the launch goes on without /btw and says why.
    """
    try:
        record = json.loads((vendor / _PI_BTW_SOURCE_NAME).read_text(encoding="utf-8"))
    except (OSError, UnicodeError, json.JSONDecodeError):
        record = None
    if not isinstance(record, dict) or not str(record.get("sha256") or ""):
        log(f"未记录构建来源（缺 {_PI_BTW_SOURCE_NAME}），本次启动不注入 /btw。")
        return False
    key = _pi_btw_integrity_key(bundle)
    if key is None:
        return False
    if key in _PI_BTW_VERIFIED:
        return True
    try:
        raw = bundle.read_bytes()
    except (OSError, MemoryError):
        log("读取不到内建 /btw 扩展文件，本次启动不注入。")
        return False
    if len(raw) > _PI_BTW_TRUSTED_BYTES:
        log(f"内建 /btw 扩展异常增大（{len(raw)} 字节），本次启动不注入。")
        return False
    digest = hashlib.sha256(raw).hexdigest()
    if digest != str(record["sha256"]).strip().lower():
        log(
            "内建 /btw 扩展与构建记录不一致（sha256 不匹配），"
            "如需更新请重新运行 scripts/sync_pi_btw.py；本次启动不注入。"
        )
        return False
    if not raw.decode("utf-8", errors="replace").startswith(_PI_BTW_BANNER):
        log("内建 /btw 扩展缺少生成标记，本次启动不注入。")
        return False
    _PI_BTW_VERIFIED.add(key)
    return True
...
def _pi_btw_package_names(path):
    """Package sources listed in one Pi settings.json, or () when unreadable."""
    try:
    ...
    for item in packages:
        if isinstance(item, str):
            ...
            source = item.get("source") or item.get("path") or item.get("name")
            if source:
                names.append(str(source))
    return tuple(str(name) for name in names)


def _pi_btw_btw_named(path):
    """Any pi-btw package source, or a file under an extensions dir named btw."""
    text = str(path).replace("\\", "/")
    if _PI_BTW_PACKAGE_MARKER in text.lower():
        return True
        ...
        try:
            if path.is_dir():
                for item in sorted(path.iterdir()):
                    if _pi_btw_btw_named(item.name):
                        return str(item)
                continue
            if not path.is_file():
                continue
            for name in _pi_btw_package_names(path):
                if _pi_btw_btw_named(name):
                    return f"{path}: {name}"
        except OSError:
            continue
    return ""
...
def pi_btw_extension_path(env=None, runtime=None, cwd=None, log=None):
    ...
    Both entrances (Pilot worker and terminal `mms`) resolve through here so the
    ...
    try:
        if not _pi_btw_preference_enabled(runtime):
            log_line("pi_btw 已关闭：不注入内建 /btw 扩展。")
            return ""
        vendor = Path(_pi_btw_vendor_dir())
        bundle = vendor / _PI_BTW_BUNDLE_NAME
        if not bundle.is_file():
            log_line(f"未找到内建 /btw 扩展（{bundle}），本次启动不注入。")
            return ""
...
def _pi_btw_preference_enabled(runtime):
    """Default-on switch; only an explicit false in the allowlist turns it off."""
    runtime = runtime if isinstance(runtime, dict) else {}
    if "pi_btw" in runtime:
        return bool(runtime.get("pi_btw"))
    try:
        overlay = _preference_runtime_overlay(load_user_preferences(), "pi")
    except Exception:
        return True
    value = overlay.get("pi_btw")
