...
def opencode_committee_agent_configs(agent_models, *, roster_config=None, agent_policies=None):
    ...
    committee_synthesis_contract = (
        ...
        "readable while still being copyable. Use exact section order: (0) 追踪块 "
        "/ Trace: begin with the unchanged MMS-MISSION block as its own visible "
        "section. (1) 人需要看的 / Human Notes: put the conclusion, "
        ...
        "requester, not the copy-forward packet. (2) 可直接复制转发 / "
        "Copy-forward Packet: keep it clean and self-contained, and include the "
        "same 追踪块 / Trace, goal, committee_policy, assignments, direct "
        "verification, member findings or ballots, tally and consensus, "
        "disagreements, risks, formal artifact status, and provenance. Do not "
        "include scorecard, meta commentary, or host private advice in this "
        "packet. (3) Host 建议 / Host Recommendation: put the host's recommended "
        "next action at the bottom of the substantive synthesis, after the "
        "copy-forward packet and before the trace footer. (4) 追踪页脚 / Trace "
        "Footer: finally, repeat MMS-MISSION plus MMS-TARGET at the very end. "
    )
    host_prompt = (
        ...
        "committee-host-pro": {
            ...
            "prompt": _prompt_with_tool_policy(
                (
                    ...
                    "synthesis), "
                    + committee_synthesis_contract
                    + "include the same task-local subagent scorecard, and "
                    "summarize only evidence-backed conclusions. Send the "
                    "unchanged mission block in every member brief. Repeat the "
                    "MMS-MISSION block at the top and MMS-MISSION plus "
                    "MMS-TARGET at the very end."
                ),
