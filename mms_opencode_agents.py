...
OPENCODE_REVIEW_MISSION_CONTRACT = (
    ...
    "include a visible mission trace in the copy-forward/provenance area or "
    "immediately after human-facing notes, and repeat at least MMS-MISSION and "
    "MMS-TARGET at the bottom so pasted findings remain traceable. The top/body "
    "trace and the bottom footer identify the same current manual dispatch; never "
    "use the top for a previous dispatch or the bottom for a future dispatch."
)
...
def opencode_committee_agent_configs(agent_models, *, roster_config=None, agent_policies=None):
    ...
    host_prompt = (
        ...
        "Synthesize only after member outputs are in. Use this final synthesis "
        "order unless the user or local project requires a stricter template. "
        "Use Simplified Chinese section titles and Chinese prose by default, while "
        "keeping technical terms such as committee_policy, MMS-MISSION, verdict, "
        ...
        "Use exact section order: (1) 人需要看的 / Human Notes: put the conclusion, "
        "advisory/formal boundary, direct verification status, material risks or "
        "dissent, and task-local subagent scorecard here; this section is for the "
        "requester, not the copy-forward packet. (2) 可直接复制转发 / Copy-forward "
        "Packet: keep it clean and self-contained; start this packet with 追踪块 / "
        "Trace containing the same current MMS-MISSION block, then include goal, "
        "committee_policy, assignments, direct verification, member findings or "
        "ballots, tally and consensus, disagreements, risks, formal artifact status, "
        "and provenance. Do not include scorecard, meta commentary, or host private "
        "advice in this packet. (3) Host 建议 / Host Recommendation: put the host's "
        "recommended next action at the bottom of the substantive synthesis, after "
        "the copy-forward packet and before the trace footer. (4) 追踪页脚 / Trace "
        "Footer: finally, repeat MMS-MISSION plus MMS-TARGET at the very end. The "
        "Trace block and Trace Footer identify the same current dispatch; do not use "
        "the top/body trace for a previous dispatch or the footer for a future dispatch. "
        "For the scorecard, rate each delegated member for this task only on a 1-5 "
...
def opencode_debate_agent_configs(agent_models, *, roster_config=None, agent_policies=None):
    ...
    host_prompt = (
        ...
        "reply should include a visible MMS-MISSION trace and stay compact: "
