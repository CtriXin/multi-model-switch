...
def opencode_committee_agent_configs(agent_models, *, roster_config=None, agent_policies=None):
    ...
    host_prompt = (
        ...
        "first begin with the unchanged MMS-MISSION block. Then write "
        "(1) Human review notes: put the conclusion, advisory/formal boundary, "
        "direct verification status, material risks or dissent, and task-local "
        "subagent scorecard here; this section is for the requester, not the "
        "copy-forward packet. (2) Copy-forward packet: label it clearly as "
        "Copy-forward packet / safe to copy/forward, keep it clean and "
        "self-contained, and include goal, committee_policy, assignments, "
        "direct verification, member findings or ballots, tally and consensus, "
        "disagreements, risks, formal artifact status, provenance, and the "
        "same MMS-MISSION block. Do not include scorecard, meta commentary, "
        "or host private advice in this packet. (3) Host recommendation: put "
        "the host's recommended next action at the bottom of the substantive "
        "synthesis, after the copy-forward packet and before the trace footer. "
        "Finally, repeat MMS-MISSION plus MMS-TARGET at the very end. In the "
