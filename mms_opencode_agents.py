...
def opencode_committee_agent_configs(agent_models, *, roster_config=None, agent_policies=None):
    ...
    committee_synthesis_contract = (
        ...
        "(2) 可直接复制转发 / Copy-forward Packet: keep it clean and "
        "self-contained. Start this packet with 追踪块 / Trace containing the "
        "same current MMS-MISSION block. Keep it compact for forwarding: include "
        "only the goal, committee_policy, direct verification summary, key "
        "findings, tally and consensus, model timing summary, formal artifact "
        "status, and provenance. Do not duplicate Human Notes; do not include "
        "scorecard, per-member score rationale, long ballot prose, meta "
        "commentary, or host private advice in this packet. (3) Host 建议 / Host Recommendation: "
    ...
    host_prompt = (
        ...
        "capture a dispatch start time when practical, then record each member's "
        "return order, elapsed wall time when captured, and speed ratio against "
        "the fastest captured member. If exact timing is unavailable, still record "
        "the observed return order and write elapsed time as not_captured. "
...
def opencode_debate_agent_configs(agent_models, *, roster_config=None, agent_policies=None):
    ...
    host_prompt = (
        ...
        "position, never the assigned advocacy; treat assigned stances as "
        "stance_authenticity=assigned and never count them as genuine convergence. "
        ...
        "debate-host-pro": {
            ...
            "prompt": _prompt_with_tool_policy(
                (
                    ...
                    "steelman, or free) while requiring final_stance to stay honest "
                    "with stance_authenticity, apply the "
    ...
    def _member_prompt(name, model_ref):
        ...
        return (
            ...
            "position regardless of any assigned_role; set stance_authenticity to "
            "honest and never present assigned advocacy as genuine conviction. "
