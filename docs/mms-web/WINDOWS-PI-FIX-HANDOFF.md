...
- 当前 release：`v4.21.13`（源代码收编中的下一版）
...
`v4.21.12` 已在 `mms_launchers.py` 增加 Windows Win32 `OpenProcess` / `GetExitCodeProcess` 存活检查；POSIX 仍使用原来的 `os.kill(pid, 0)`。Windows 用户已在该版本完成 acceptance。后续源代码收编还包括 artifact preview、共享 `fd/rg`、skills 路径和 traceback 日志修复，计划随 `v4.21.13` 发布。
