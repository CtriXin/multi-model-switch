...
| 成果 | 基础 Markdown/文本文件展示与工作文件查看 | 还不是完整 HTML/PDF/Office/图片成果系统 |
...
- `files.py` / `artifacts.py`：显式文件引用与可呈现成果。文件是用户工作内容，不是配置隔离目录的附属物。
...
已有 MMF Registry 的模型/默认effort管理与纯Web独立配置仍有差别：后者可连接服务、选择模型及本次effort，但没有同一个长期默认effort管理入口。后续应让Web独立配置在自己的state-root中采用同一Registry写入/发布合同，保留预览、回读与来源隔离，不迁移或自动写用户真实MMF根目录。先补新用户连续两次修改/重启验证，再扩默认设置。
