...
v4.2.0 已发布：用户主动保存项目资料，可编辑、停用和删除，按项目分区；新消息读取当前已启用版本，记录真实来源与发送状态。停用/删除只阻止后续再次加入，不承诺清除历史上下文。「本次使用」展示 Web 明确加入请求的资料、Skills、文件引用、附件和成果选段；缺少原生加载证据时不标为已加载。不做自动长期记忆，不写全局 MMS 配置。
...
本轮实施范围：升级 v2 模板，保留 v1 导入；导出先可编辑预览和最终隐私检查，导入填写必要变量后进入草稿。按当前项目匹配真实 Skills ID，检查所选模型能力并要求明确确认，后台刷新不能替代这次确认。没有账户配置搬运、自动安装和自动执行。验证计划见原 Stride task 855b1bed42aa43bb。
