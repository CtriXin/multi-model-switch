...
本轮按用户要求优先完成通道绑定的悬浮引导，并收尾第 3、4 项。**六项路线中，前四项的首轮范围已经实现；第 5、6 项仍未实施。** 发布、main 晋级和本机运行身份以原任务最终交付回执为准，不能仅凭源码版本判定升级完成。

| 顺序 | 迭代内容 | 当前结果 | 边界 |
|---|---|---|---|
| 1 | 新用户配置收口 | v4.0.1 默认 effort 闭环；v4.10 连接地址、Key、模型、保存和预设均由悬浮卡指向真实控件，完成后进入聊天教程 | 不自动迁移真人 MMF 配置；只有点击保存才改连接，只有点击发送才调用模型 |
| 2 | 成果预览与继续修改 | v4.1 Markdown、文本、图片、CSV、静态 HTML、命令成果发现、选段与版本比较 | 扫描/格式/容量有上限；不是全盘监听、文件回滚或可执行应用预览 |
| 3 | 上下文与 Skills 可检查 | 项目资料可编辑/停用/删除；v4.10 加入 Pi 实际规则与 Skills 来源、所选正文加载、明确 Skill 命令/read 调用及失败、按条记录与恢复 | 不将“可用”冒认为“已使用”；旧 Pi、旧历史和无法归因的间接读取明确无证据，不推测模型是否遵循 |
| 4 | 任务模板真正可复用 | v2 目标、示例、变量、Skills/模型要求；导出清理与预览、导入草稿、变量/编辑恢复、v1 兼容；三份即用示例 | 不搬运账号/文件，不自动安装依赖。导出清理只覆盖已知模式，仍需检查业务内容 |
| 5 | 新增一个 harness | 未接入；Web 实际执行仍是 Pi | 另起一轮验证 Codex 或 OpenCode 的完整生命周期，不能把安装 CLI 当作 Web adapter |
| 6 | 扩大产品形态 | 未实施 | desktop、可见浏览器操作、PDF/Office 需真实试用支持 |

### 本轮收尾证据

- 基于 Claude 已合并的 PR142 / dev 6702c41b；保留 Markdown 更新说明、文件夹间距和有新版才显示更新按钮，核验设置版本入口与弹窗返回。
- 通道向导验证空值、错误 Key、返回保留、模型未选择拦截、保存预设、教程衔接、问号重开、Escape、草稿保留；390/1024/1280/1600px、深色和大字布局。中等宽度的卡片遮挡已修复。
- Pi `before_agent_start.systemPromptOptions` 提供规则与 Skill 目录来源；只保存路径/指纹。消费的 prompt hash 对应原消息，工具 ID 保留读取与失败归属，不让排队消息串用证据。
- 模板旧稿四个审查问题已修复：带引号的凭据字段、v1 字面变量恢复、最终 JSON 字节预算、版本和未知能力严格校验。发送前前后端核验模型/Skills；网络预检中项目或模型往返切换也使旧确认失效。
- 原任务保留本机 MiniMax fixture + 真实 Pi、正常浏览器交互、归档安装/恢复及测试日志。自动测试与页面验收不能替代 3–5 位新用户的独立试用和第二天继续。

### 下一轮顺序

1. **独立新用户试用**：请未参与开发的人用三份内置模板，从安装、连接、处理文件、看成果到第二天继续。记录要维护者解释的步骤，由用户安排，不自动联系他人。
2. **按试用巩固 MMF 的优势**：模型/通道、能力和 effort、失败原因、换通道继续、升级保留工作。前四项不再从零重做，也不以增加菜单代替解决真实问题。
3. **单个新 harness**：Codex 或 OpenCode 二选一，先完整验证启动、审批、中断、恢复、成果与账号隔离。插件市场、自动长期记忆和 desktop 继续后置。

npm/npx 按用户要求等待登录后再处理；源码包含安装入口不代表 npm registry 已发布。固定 [v4.0.0 方案](https://github.com/CtriXin/multi-model-switch/blob/v4.0.0/docs/mms-web/NEXT-PHASE.md) 保留历史，不改旧 tag。原 Stride task 855b1bed42aa43bb 继续保存唯一进度、证据和回归记录。
...
| 分享做法 | 模板 v2 变量、示例、需求校验与草稿导入/导出 | 不是完整的插件/依赖/模型账号安装包 |
...
v4.2.0 已公开交付上述首轮资料管理：用户主动保存项目资料，可编辑、停用和删除，按项目分区；新消息读取当前已启用版本，记录真实来源与发送状态。停用/删除只阻止后续再次加入，不承诺清除历史上下文。来源清单现名为「附带资料」，只展示 Web 明确加入请求的资料、Skills、文件引用、附件和成果选段；普通消息没有额外资料时不展示空清单。v4.10 补齐 Pi 可观察的规则加载、Skill 目录、正文加载、明确命令和 read 调用/失败；不能归因的读取和旧历史仍明确无证据。不做自动长期记忆，不写全局 MMS 配置。


### 3. 把成功做法分享出去

v4.10 已将任务模板升级为：目标、示例输入/成果、Skills 需求、模型能力要求、必要变量。导入先变为可修改草稿，缺少能力时清楚提示；发送才开始实际执行。预览并移除 Key、环境变量、私有历史、原机器绝对路径。
