...
没有可用通道时先完成连接，配置成功后才显示聊天功能教程。空白安装会直接打开连接向导，悬浮提示逐个圈亮需要填写或确认的控件；已有通道暂时不可用时，打开设置查看原因，不要求重新创建账号。
...
没有服务信息时可以「稍后配置」；? →「引导我连接新通道」可随时重开。Escape 先收起悬浮说明，再按一次关闭表单。API Key 不存入浏览器；关闭未保存向导会清理临时预览。读取模型列表不代表已验证生成能力；实际对话使用服务额度。
...
- 工作文件夹：点击新会话顶部的文件夹，输入项目名查找已添加的项目；检测到 zoxide 时，也会利用它的常用目录记录。可以直接按 Enter 选中，或在弹窗里浏览文件夹、粘贴完整路径。Pilot 记住上次选择；没有 zoxide 也可正常使用，无需安装额外依赖。AI 会在这个目录工作，这不代表操作系统级 sandbox。
...
- 模型切换：点击输入框右下角模型名，搜索并点选即可切换，使用已设定的默认通道；展开「通道与高级选项」可临时修改通道、effort 和工作方式。同通道可原生切换；不同通道通过新私有进程延续原会话。保留历史，下一条消息采用新选择。正在执行或等待确认时，先完成/停止本轮再切换。
- effort：选择所选模型真实支持的级别；「默认」来自 MMS 已配置的默认值。不同模型可用级别不同。
- Skills 与命令：添加 Skills，使用 `/` 打开命令提示，`@` 查找文件。只展示当前接入支持的命令与交互。
- 过程与结果：工具按组展示，支持展开参数和输出；最后回答保留在外面。每轮可收起/展开过程，底部可收起/展开全部。设置 → 使用 →「完成后自动收起过程」控制默认行为，设置保存在当前浏览器。
...
默认地址是 `http://127.0.0.1:8765`，只在运行服务的本机可访问。把这个地址发给别人，他们打开的是自己的电脑，不是你的服务。v4 没有多用户账号、团队权限，也不托管任何公网服务。

你自己的手机或另一台电脑要访问同一个 Pilot，在设置 → 外观与使用里打开「让手机或另一台电脑访问」，页面会给出带 token 的链接和二维码。默认关闭，关掉后对外端口立刻关闭。出门访问是同一个开关打开之后的副选项：已有域名贴进隧道域名，没有通道再交给 Pilot 配。我们不提供通道。两条路的做法和边界都写在 [REMOTE-ACCESS.md](REMOTE-ACCESS.md)。

若只是分享工作方法，用会话菜单的「保存为任务模板」编辑目标、示例、Skills 和模型能力要求，把每次不同的内容写成 `{{file}}` 等变量。检查导出预览后再下载；对方导入后填写自己的资料，核对项目与模型，再发送。模板不是 Key/账号搬家工具，也不是自动打包所有 Skills 的插件安装包。分享示例材料前仍应自行检查业务信息。
...
升级前先等正在执行的任务完成，并处理等待确认的任务，再按该版本安装说明更新。数据目录与安装目录分开，安全更新会等待活跃进程结束，执行中、待确认或排队时不会强行切换。允许重启空闲会话是可选项；这不等于把运行中的进程无缝搬到新版本。重启后历史可打开，继续发送会用保存的上下文启动新的 Pi 进程。

必须保留原 `--state-root`、`--config-root`（如果指定过），以及整个数据目录中的 `sessions/`、`runtimes/` 和项目文件；不要只备份聊天列表。`runtimes/` 包含继续会话需要的运行快照与原生记录。手动改目录不会自动搬迁旧会话。更新前备份数据；可在 ? 旁的「更新」中检查并安全升级，详情见[升级与会话保护](UPGRADE-SAFETY.md)。v3 没有 Web，不能用 v3 打开 v4 会话界面。
...
消息附带文件、Skills 或项目资料时，下方会显示默认折叠的「附带资料」，需要时再查看版本、来源和引用内容。普通聊天没有额外资料时不显示空清单。提交记录不代表模型已经读取文件；排队、取消、发送失败或结果待确认仍单独提示，旧消息没有来源记录时不会补造。
...
v4.10.0 起，首次连接的地址、Key、模型和保存也有贴着真实控件的悬浮引导；连接成功并读取模型预设后才进入聊天教程。已有可用通道直接介绍聊天操作，跳过已完成的连接步骤。亮起的功能可以直接点击：工作文件夹 → 快速选择模型 → 可选 effort → 第一条消息 → 手动发送并查看回复。工作文件夹和 effort 可保留默认值。

引导里的「帮我填入一条示例」会保留新任务已有草稿，把示例追加在后面。你可以修改它；只有自己点击发送才会调用模型。只有你操作对应控件才会改变模型或 effort；点击保存通道才保存连接，点击发送才执行任务。打开模型选择、Skills 等真实窗口时，引导会暂时让开，关闭后恢复。
...
v4.5.0 起，通道模型管理的「检查最新能力」会逐行显示差异：先勾选、填入草稿，再检查并保存。你已保存或尚未保存的手动值默认不勾；也可主动查询 OpenRouter 目录作为备选，它不会越过仓库和本地已知资料。追加查询不会重置已经做出的选择。模型能力数据随 MMS Release 更新；设置中的版本号可手动检查更新；默认定时检查，? 旁在发现新版时显示入口，并提供安全升级。升级应用与「检查最新能力」是两个步骤；模型建议仍需确认后保存，不会自动覆盖人工设置。


### 不会安装 skill，也能开始

v4.7.0 起，点输入框旁的 Skills，默认显示五个内置用途：把想法变成步骤、检查当前改动、保存进度、继续上次、整理交接。无需安装；点“用示例填入草稿”会选中能力并保留原有文字，补充要求后发送才执行。

“全部能力”和搜索包含已有的项目、共享 skills，同名自定义能力不会被覆盖。外部 skills 的目录与来源说明可在窗口下方展开。Pilot 不会自动下载陌生脚本或把这些指引写进其他 Agent 的全局目录。

保存进度与交接优先使用原任务记录；没有任务系统时使用项目内 `.agent.local/continuity/`，无需额外安装。继续上次会核对文件与记录，找不到记录时会说明。保存的是工作说明与证据，不是进程快照，也不会关闭会话。
...
新会话的「试用任务模板」提供整理产品需求、检查项目改动、分析一份数据。填写变量会先生成可编辑草稿；未填值、缺必需 Skill、模型能力不足时会说明原因。换模型或项目需要重新核对，发送前服务端还会检查实际配置。

导入模板不会自动安装 Skill、读取文件、改通道或执行任务。变量填写和载入后的编辑可以在当前浏览器刷新恢复；旧版 v1 模板中的 `{{文字}}` 按原文保留。外部模板来源需自行判断。导出只包含白名单字段，清理已识别的凭据、环境赋值和本地绝对路径；下载前仍须检查是否有不宜公开的业务信息。
