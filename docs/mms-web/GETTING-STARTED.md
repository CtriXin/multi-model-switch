...
v4.6.0 起，首次没有可用通道时先打开分步连接流程，连接成功并读取模型预设后才显示悬浮教程。已有可用通道直接介绍聊天操作，跳过已完成的连接步骤。亮起的功能可以直接点击：工作文件夹 → 快速选择模型 → 可选 effort → 第一条消息 → 手动发送并查看回复。工作文件夹和 effort 可保留默认值。
...
v4.5.0 起，通道模型管理的「检查最新能力」会逐行显示差异：先勾选、填入草稿，再检查并保存。你已保存或尚未保存的手动值默认不勾；也可主动查询 OpenRouter 目录作为备选，它不会越过仓库和本地已知资料。追加查询不会重置已经做出的选择。模型能力数据随 MMS Release 更新；? 旁的「更新」会定时检查新版，并提供安全升级。升级应用与「检查最新能力」是两个步骤；模型建议仍需确认后保存，不会自动覆盖人工设置。
