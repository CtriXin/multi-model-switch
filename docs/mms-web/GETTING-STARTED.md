...
你自己的手机或另一台电脑要访问同一个 Pilot，在设置 → 外观与使用里打开「让手机或另一台电脑访问」，页面会给出带 token 的链接和二维码。默认关闭，关掉后对外端口立刻关闭。出门访问需要自己搭一条通道，我们不提供支持。两条路的做法和边界都写在 [REMOTE-ACCESS.md](REMOTE-ACCESS.md)。
