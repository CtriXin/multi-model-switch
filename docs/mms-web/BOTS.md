...
不足两家时走 direct，理由是「现在只有一家能用，我自己看了，不是多方评审。」同家族只留一个通道。家族判定优先用 catalog 的 `family`，兜底只走 `mms_core.MODEL_FAMILIES`，不再另写一份前缀表。第一次真正扇出时计划块给一句可见提示（并行听 N 家、可关掉），之后不再重复。
