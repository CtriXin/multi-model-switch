...
用户跟**自己起名的那一个 Bot**说话即可。关掉时输入框上只留「寻求场外帮助」。打开后选听听或问仔细，发出去就会再问几家。悬停只给短摘，点开才看完。

默认开、强度「听意见」、**2 家便宜对照**。点家族芯片可多选；全选后再点是全不选。悬停家族可选具体型号并记住。子任务过程不进主聊天。

自动选择不足两家时走 direct，并明确说明不是多方评审。显式选择的家族或固定型号失效时停止并提示重新选择，不会换家族或换同名型号；显式只选一家时提示至少选两家或关闭场外帮助。型号菜单支持清除固定型号，回到该家族自动选择。同家族只留一个通道。家族判定优先用 catalog 的 `family`，兜底只走 `mms_core.MODEL_FAMILIES`，不再另写一份前缀表。第一次真正扇出时计划块给一句可见提示（并行听 N 家、可关掉），之后不再重复。汇总默认只摊开判断、分歧和风险，共识和各家原文收起来。

Fleet worker 只能由 coordinator 内部创建，使用指定模型的一次性只读 Pi session：仅开放 read、grep、find、ls，禁用自动扩展、skills 和上下文文件加载，不带 Bot 内部命令权限，不接受续聊、切换模型或创建会话分支。它不修改主 Bot 的模型、会话或待生效模型，不将各家意见写入共享长期记忆；最终综合仍按主 Bot 的记忆设置保存。完成或失败后回收临时进程，启动失败也会进入汇总，不无限等待。

即时发送、一次定时和循环日程都保存场外帮助意图。日程执行时使用 Bot 届时的 Fleet 开关、家族与型号配置，并非创建时的模型快照；关闭开关后按普通任务执行。旧日程不自动打开 Fleet。`plan-approve` 保留确认与拒绝入口。
