...
**这不是"覆盖率不够"。** 这是一类**特定写法的测试在系统性地认证错误的东西**,已经在这个仓库反复出现。两次合并后复查,一共抓到 **12 条**(另有 1 条查实是误报)mutation:把生产代码掏空,整套测试仍然绿。**没有一条是机器抓到的。**
...
- 提 PR(A 部分 base `main`,B 部分 base `dev`),**不要** merge
- 交付里逐条列出第二节那 12 行的红/绿 —— **这是这个包唯一的验收标准**
