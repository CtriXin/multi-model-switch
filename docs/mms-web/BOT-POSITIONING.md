...
MMS Bot 是 MMS Pilot 的正式执行层升级，不是额外的装饰功能。Pilot 的普通 Web 会话面向“我现在直接做这件事”；Bot 面向“我交代目标，由一个有身份的执行者持续完成、分发、等待、记忆并回报”。多模型评审是同一个 Bot 换几颗脑子，不是让用户再雇一排永久同事；名字由用户自己起。默认听 2 家便宜意见，聊天里一键可关、可改强度和家族。
