...
- `mmf preview doctor [--json]` and `mmf config doctor [--json]` are read-only "what next?" commands for preview setup. They check preview root mode, registry DB, legacy import candidates, latest-approved bundle verification, runtime readiness, missing API keys, missing route base URLs, and then print one next action. Missing route URLs point to a preview import + publish + verify rebuild command; missing keys point to explicit `--include-secrets` import.
