...
- `mmf preview doctor --strict-exit` and `mmf config doctor --strict-exit` exit non-zero unless the preview root is runtime-ready. This avoids treating "printed something and did not crash" as success through either entry.
