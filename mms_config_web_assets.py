...
<div class="shell">
  ...
  <main class="content">
    ...
    <section class="panel" data-section="sessionAssets">
      <h2>Skill / MCP 管理</h2>
      <p>这里就是会话能力的配置入口：筛选 Skill / MCP / Hook，勾选默认关闭，复制偏好片段后应用到 preferences.toml。</p>
...
function renderReviewSummary(plan){const review=plan?.review_summary||{};const counts=review.counts||{};const risks=review.risks||[];const items=review.items||[];const configItems=items.filter(item=>item.kind!=='no_change');const assetDiff=assetDraftDiff();const riskHtml=risks.length?`<h4>风险提示</h4><div>${risks.map(r=>`<p><span class="tag ${r.level==='danger'?'off':''}">${escapeHtml(levelLabel(r.level))}</span> <strong>${escapeHtml(r.title)}</strong> ${escapeHtml(r.detail)}</p>`).join('')}</div>`:'<p><span class="tag">无高风险提示</span></p>';let itemHtml=configItems.length?configItems.map(item=>`<p><span class="tag ${item.level==='danger'?'off':''}">${escapeHtml(levelLabel(item.level))}</span> <strong>${escapeHtml(item.title)}</strong> ${escapeHtml(item.detail)}</p>`).join(''):'<p class="muted">模型/通道配置没有变化。</p>';if(assetDiff.total){itemHtml+=`<p><span class="tag">能力草稿</span> <strong>Skill/MCP 默认关闭草稿有 ${assetDiff.total} 项</strong> 这不会通过“保存审计”写入；请回到 Skill / MCP 管理里的“应用默认关闭”，复制 preferences.toml 片段并按 HumanGate 应用。</p>`}$('reviewSummary').innerHTML=`<div class="chips"><span class="chip">配置变化 ${configItems.length}</span><span class="chip">能力草稿 ${assetDiff.total}</span><span class="chip">风险 ${counts.risks||0}</span><span class="chip">移除隐藏记录 ${counts.hidden_removed||0}</span><span class="chip">凭据更新 ${counts.credential_updates||0}</span></div>${riskHtml}<h4>将要写入的变化</h4>${itemHtml}${planJsonHint(plan)}`}
