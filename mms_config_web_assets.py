    ...
    <section class="panel" data-section="sessionAssets">
      ...
      <div class="grid asset-ops-grid">
        <div class="card span6 asset-config-card asset-ops-card">
          ...
          <label class="check"><input id="assetConfirmPrefs" type="checkbox"><span>确认只写 Skill/MCP/Hook 偏好，不改模型和通道</span></label>
          <label>确认文字</label>
          <input id="assetConfirmPhrase" placeholder="保存偏好">
          <div class="btns">
            <button id="applyAssetPrefs" class="secondary">应用到 preferences.toml</button>
            <button id="copyAssetPrefs" class="ghost">复制偏好片段</button>
            <button id="resetAssetPrefs" class="ghost">恢复当前偏好</button>
          </div>
          <p class="muted" id="assetPrefsResult">不会走模型配置审计；这里有单独的 preferences backup + audit。</p>
          <details>
...
function bindAssetPreferenceButtons(){const copy=$('copyAssetPrefs');const reset=$('resetAssetPrefs');const apply=$('applyAssetPrefs');if(copy)copy.onclick=async()=>{const snippet=renderAssetPreferenceSnippet();try{await navigator.clipboard.writeText(snippet);toast('偏好片段已复制')}catch(_err){toast('无法访问剪贴板，片段已显示在页面')}};if(reset)reset.onclick=()=>{assetDisabledDraft=cloneAssetDisabledDefaults();renderSessionAssets();toast('已恢复为当前 preferences.toml 状态')};if(apply)apply.onclick=async()=>{const payload=assetPreferencesPayload();const data=await api('/api/preferences/apply',{...payload,confirm_preferences:$('assetConfirmPrefs')?.checked,confirm_phrase:$('assetConfirmPhrase')?.value,reason:'setup-web-ui:asset-preferences'});const box=$('assetPrefsResult');if(box)box.textContent=data.ok?(data.status==='no_change'?'偏好没有变化。':`已写入 ${data.target_path||'preferences.toml'}；backup ${data.backup_path||'-'}`):(data.errors||[data.error||'保存被阻止']).join('；');toast(data.ok?(data.status==='no_change'?'偏好没有变化':'Skill/MCP 偏好已保存'):'Skill/MCP 偏好保存被阻止');if(data.ok){const res=await fetch('/api/state');state=await res.json();assetDisabledDraft=null;renderAll()}}}
function renderAssetControlHelp(){const box=$('assetControlHelp');if(!box)return;const assets=state.session_assets||{};const contract=assets.configuration_contract||{};const install=assets.managed_install||{};const rows=assetRows();const canClose=rows.filter(row=>assetDisableSupported(row)).length;const globalCanClose=rows.filter(row=>row.group==='global'&&assetDisableSupported(row)).length;const prefPath=contract.persistent_path||'~/.config/mms/preferences.toml';const managedRoot=install.root||contract.managed_assets_root||'~/.local/share/mms/assets';box.innerHTML=`<article class="asset-control-card"><h3>在这里开 / 关</h3><p class="muted">卡片右侧的“默认关闭”会进入草稿；点击“应用到 preferences.toml”后，下一次 MMS 启动生效。</p><p><span class="tag">可关闭 ${canClose}</span><span class="tag">Global 可关闭 ${globalCanClose}</span></p><div class="btns"><button class="ghost" data-asset-jump-tab="mms_dynamic" data-asset-jump-kind="all">管理 MMS 动态</button><button class="ghost" data-asset-jump-tab="global" data-asset-jump-kind="skills">管理 Global Skill</button><button class="ghost" data-asset-jump-tab="all" data-asset-jump-kind="mcp">管理 MCP</button></div></article><article class="asset-control-card"><h3>添加到 MMS 动态</h3><p class="muted">固定放到 MMS managed assets root；推荐放 symlink，不复制大包。launcher 会优先读这里，再回退 vendor / 历史路径。</p><p class="mono">${escapeHtml(managedRoot)}</p><p class="muted">偏好文件：${escapeHtml(prefPath)}</p><div class="btns"><button class="ghost" data-asset-open-sources="1">查看当前加载路径</button></div></article><article class="asset-control-card"><h3>添加到 Global</h3><p class="muted">想让原生 CLI 也能看到，就放到对应全局目录；本页可帮 MMS 启动时默认关闭 Claude/Codex Global Skill。</p><p class="mono">Claude: ~/.claude/skills/&lt;name&gt;</p><p class="mono">Codex: ~/.codex/skills/&lt;name&gt;</p></article>`;document.querySelectorAll('[data-asset-jump-tab]').forEach(btn=>{btn.onclick=()=>{assetTab=btn.dataset.assetJumpTab||assetTab;assetKind=btn.dataset.assetJumpKind||assetKind;assetCli='all';renderSessionAssets();toast('已切换管理范围')}});document.querySelectorAll('[data-asset-open-sources]').forEach(btn=>{btn.onclick=()=>{const diag=document.querySelector('#assetManagedRoots details.asset-source-diagnostic');if(diag){diag.open=true;diag.scrollIntoView({block:'nearest',behavior:'smooth'});toast('已展开当前加载路径')}}})}
...
function renderReviewSummary(plan){const review=plan?.review_summary||{};const counts=review.counts||{};const risks=review.risks||[];const items=review.items||[];const configItems=items.filter(item=>item.kind!=='no_change');const assetDiff=assetDraftDiff();const riskHtml=risks.length?`<h4>风险提示</h4><div>${risks.map(r=>`<p><span class="tag ${r.level==='danger'?'off':''}">${escapeHtml(levelLabel(r.level))}</span> <strong>${escapeHtml(r.title)}</strong> ${escapeHtml(r.detail)}</p>`).join('')}</div>`:'<p><span class="tag">无高风险提示</span></p>';let itemHtml=configItems.length?configItems.map(item=>`<p><span class="tag ${item.level==='danger'?'off':''}">${escapeHtml(levelLabel(item.level))}</span> <strong>${escapeHtml(item.title)}</strong> ${escapeHtml(item.detail)}</p>`).join(''):'<p class="muted">模型/通道配置没有变化。</p>';if(assetDiff.total){itemHtml+=`<p><span class="tag">能力草稿</span> <strong>Skill/MCP 默认关闭草稿有 ${assetDiff.total} 项</strong> 这不会通过“保存审计”写入；请回到 Skill / MCP 管理里的“应用默认关闭”，点击“应用到 preferences.toml”。</p>`}$('reviewSummary').innerHTML=`<div class="chips"><span class="chip">配置变化 ${configItems.length}</span><span class="chip">能力草稿 ${assetDiff.total}</span><span class="chip">风险 ${counts.risks||0}</span><span class="chip">移除隐藏记录 ${counts.hidden_removed||0}</span><span class="chip">凭据更新 ${counts.credential_updates||0}</span></div>${riskHtml}<h4>将要写入的变化</h4>${itemHtml}${planJsonHint(plan)}`}
