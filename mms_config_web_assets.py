  ...
  <main class="content">
    ...
    <section class="panel" data-section="sessionAssets">
      ...
      <div class="grid">
        <div class="card span6">
          <h3>配置写到哪里</h3>
          <p class="muted" id="assetConfigContract">WebUI 当前只读展示能力目录；持久默认值建议写 preferences.toml。</p>
...
let state=null; let activeProvider=0; let activeProviderTab='config'; let lastPlan=null; let opencodeAgentFilter="all"; let opencodeOnlyOverridden=false; let editingExtraModels=false; let touchedProviders=new Set(); let staleCleanupProviders=new Set(); let assetTab='mms_dynamic'; let assetCli='all'; let assetKind='all';
...
function renderSessionAssets(){if(!$('assetTable'))return;const assets=state.session_assets||{};const summary=assets.summary||{};const contract=assets.configuration_contract||{};$('assetSummary').innerHTML=`<div class="card span4"><h3>MMS dynamic</h3><p class="muted">session-local 注入，不改 global config。</p><span class="tag">${summary.mms_dynamic||0} items</span><span class="tag">skills ${summary.skills||0}</span></div><div class="card span4"><h3>Global / inherited</h3><p class="muted">来自 CLI 全局配置或已安装插件；这里只读。</p><span class="tag ${summary.global?'':'off'}">${summary.global||0} items</span><span class="tag">MCP ${summary.mcp||0}</span></div><div class="card span4"><h3>Hooks</h3><p class="muted">自动行为主要看 hooks；skills 多数是可调用能力。</p><span class="tag">${summary.hooks||0} hooks</span><span class="tag">total ${summary.total||0}</span></div>`;renderAssetFilters();let rows=assetRows().filter(row=>(assetTab==='all'||row.group===assetTab)&&(assetCli==='all'||row.cli===assetCli)&&(assetKind==='all'||row.kind===assetKind));$('assetTable').innerHTML=`<thead><tr><th>CLI</th><th>Surface</th><th>来源</th><th>名称</th><th>说明</th><th>路径 / 触发</th><th>开关 key</th></tr></thead><tbody>${rows.length?rows.map(row=>{const path=assetDetail(row,'路径')||assetDetail(row,'Path')||assetDetail(row,'URL')||assetDetail(row,'触发')||assetDetail(row,'Trigger');const groupLabel=row.group==='mms_dynamic'?'MMS dynamic':(row.group==='global'?'Global':'Other');return `<tr><td>${escapeHtml(row.cli_label||row.cli)}</td><td><span class="tag">${escapeHtml(row.kind)}</span><br><span class="muted">${escapeHtml(row.scope_label||row.scope)}</span></td><td><span class="tag ${row.group==='global'?'off':''}">${escapeHtml(groupLabel)}</span><br><span class="muted">${escapeHtml(row.origin||'')}</span></td><td class="mono">${escapeHtml(row.title||'')}</td><td>${escapeHtml(row.summary||'')}</td><td class="mono">${escapeHtml(path||'-')}</td><td class="mono">${escapeHtml(row.disable_key||'-')}</td></tr>`}).join(''):'<tr><td colspan="7" class="empty-row">没有匹配的能力条目</td></tr>'}</tbody>`;$('assetConfigContract').textContent=`持久默认值：${contract.persistent_path||'~/.config/mms/preferences.toml'}；${contract.launch_override||''} ${contract.webui_write_scope||''}`;$('assetPreferenceSnippet').textContent=assets.preference_snippet||'';$('assetGlobalRoots').innerHTML=(assets.global_roots||[]).map(root=>`<p><span class="tag ${root.exists?'':'off'}">${root.exists?'exists':'missing'}</span> <span class="mono">${escapeHtml(root.path)}</span><br><span class="muted">${escapeHtml(root.label)}${root.skill_count?` · skills ${root.skill_count}`:''}</span></p>`).join('')||'<p class="muted">无 global roots 信息</p>'}
