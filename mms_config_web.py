...
function renderSaveControls(){const root=(state.model_source_status||{}).root||{};const preview=root.mode==='preview';if($('saveBtn')){$('saveBtn').disabled=preview;$('saveBtn').title=preview?'Preview root 已禁用 legacy save，请使用写入预览 DB + 发布':''}if($('applyV2Preview')){$('applyV2Preview').disabled=!preview;$('applyV2Preview').title=preview?'':'Stable root 不能写 preview DB，请用 mmf preview root'}}
