...
# 补充二：最新旁问默认展开，历史旁问降权（同日）

用户反馈：刚问完的那条一结束就折叠，等于每次都要再双击一次才能读；五条同权重堆在一起太重。

## 改动（只在 `apps/mms-web/src`）

- `defaultExpanded(row, newest)`：生成中展开不变；结束后**最新一条保持展开**，被下一条顶掉时才折叠。用户的显式双击/键盘切换仍然覆盖默认值。
- 折叠行降权：单行 32px，无填充背景，2px 细左边线，文字用 `--muted`，左侧圆角改为 0 让边线读作一条规则而不是圆头色块。展开卡片才拿到填充、完整描边和 accent 左边线。
- 折叠行不再重复「不影响主任务」，这句话移到展开态和 `aria-label`，屏幕阅读器仍然听得到。
- 「查看」「取消旁问」按钮只在展开时渲染，所以折叠卡片真的是一行；进行中的旁问即使被手动折叠也保留取消入口。
- 失败行即使折叠也保留红色左边线，扫读时仍然找得到。

## 执行验证

```text
apps/mms-web $ npx tsc --noEmit                 通过
apps/mms-web $ node --test "tests/*.test.mjs"   32 passed（新增 1 条，拆分 defaultExpanded 断言）
apps/mms-web $ npm run build                    ✓
```

浏览器（隔离实例 8879，5 条真实旁问记录）：

| 检查 | 结果 |
|---|---|
| 默认状态 | 前 4 条 `aria-expanded=false`、高度 32px；最新一条 `true`、高度 148px |
| 双击历史行 | 第 1 条展开到 140px，其余不变，最新一条仍展开 |
