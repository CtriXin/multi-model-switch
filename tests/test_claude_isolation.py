...
def test_backfill_project_store_resume_files_cross_account_and_path_alias(monkeypatch, tmp_path, scoped_store):
    ...
    from mms_claude.project_store import claude_raw_entry_path, ensure_claude_project_store

    project_dir = tmp_path / "repo"
    work_dir = project_dir / "work"
    work_dir.mkdir(parents=True)
    monkeypatch.setattr(mms_launchers, "canonical_project_path", lambda _path=None: str(project_dir.resolve()))
    monkeypatch.setattr(project_store, "canonical_project_path", lambda _path=None: str(project_dir.resolve()))
...
def test_claude_resume_scope_preserves_storage_slug_but_resume_is_project_scoped(monkeypatch, tmp_path):
    import mms_launchers
    ...
    assert result == "session-oauth"


def test_project_scoped_resume_backfill_copies_project_store_across_accounts(monkeypatch, tmp_path):
    import mms_launchers
    import mms_claude.project_store as project_store
    ...
    assert (target / "-tmp-repo" / "session-oauth.jsonl").exists()
