...
def test_install_script_uses_npm_first_cli_installs():
    text = INSTALL_SCRIPT.read_text(encoding="utf-8")
    installer_text = (ROOT_DIR / "mms_launcher" / "installer.py").read_text(encoding="utf-8")
...
def test_install_script_copies_launcher_package_directories():
    ...
    assert "for package_dir in mms_opencode mms_codex mms_claude mms_agy mms_pi mms_session mms_launcher mms_runtime mms_display mms_registry mms_commands mms_config; do" in text
    assert 'copy_dir_safely "$SOURCE_DIR/$package_dir" "$MMS_HOME/$package_dir"' in text
    assert 'for stale_module in "$MMS_HOME"/mms_opencode_*.py "$MMS_HOME"/mms_codex_*.py "$MMS_HOME"/mms_claude_*.py "$MMS_HOME"/mms_agy_*.py "$MMS_HOME"/mms_pi_*.py "$MMS_HOME"/mms_session_*.py "$MMS_HOME"/mms_hook_commands.py "$MMS_HOME"/mms_mmc_launch.py "$MMS_HOME"/mms_bridge.py "$MMS_HOME"/mms_command_tools.py "$MMS_HOME"/mms_config_web.py "$MMS_HOME"/mms_config_web_assets.py "$MMS_HOME"/mms_config_web_server.py "$MMS_HOME"/mms_config_web_settings.py "$MMS_HOME"/mms_tui.py "$MMS_HOME"/mms_tui_settings_actions.py "$MMS_HOME"/mms_tui_launcher_entry.py "$MMS_HOME"/mms_tui_launcher_flow.py "$MMS_HOME"/mms_openrouter_extension.py "$MMS_HOME"/mms_health_cache.py "$MMS_HOME"/mms_host_context.py "$MMS_HOME"/mms_consumer_bundle.py "$MMS_HOME"/mms_toon.py "$MMS_HOME"/mms_account_guard.py "$MMS_HOME"/mms_events.py "$MMS_HOME"/mms_project_store.py "$MMS_HOME"/mms_i18n.py "$MMS_HOME"/mms_account_env.py "$MMS_HOME"/mms_speed_stats.py "$MMS_HOME"/mms_usage.py "$MMS_HOME"/mms_context.py "$MMS_HOME"/mms_fake_upstream.py "$MMS_HOME"/mms_installer.py "$MMS_HOME"/mms_review_launch.py "$MMS_HOME"/mms_native_fallback.py "$MMS_HOME"/mms_capability_resolver.py "$MMS_HOME"/mms_provider_profiles.py "$MMS_HOME"/mms_broker.py "$MMS_HOME"/mms_adapter_registry.py "$MMS_HOME"/mms_account_state.py "$MMS_HOME"/mms_state_io.py "$MMS_HOME"/mms_rescue.py "$MMS_HOME"/mms_router.py "$MMS_HOME"/mms_launcher_*.py "$MMS_HOME"/mms_runtime.py "$MMS_HOME"/mms_runtime_*.py "$MMS_HOME"/mms_launch_display.py "$MMS_HOME"/mms_model_display.py "$MMS_HOME"/mms_confirm_preview.py "$MMS_HOME"/mms_registry.py "$MMS_HOME"/mms_registry_*.py; do' in text
