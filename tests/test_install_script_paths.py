...
def test_install_script_copies_launcher_package_directories():
    ...
    assert "for package_dir in mms_opencode mms_codex mms_claude mms_agy mms_pi mms_session; do" in text
    assert 'copy_dir_safely "$SOURCE_DIR/$package_dir" "$MMS_HOME/$package_dir"' in text
    assert 'for stale_module in "$MMS_HOME"/mms_opencode_*.py "$MMS_HOME"/mms_codex_*.py "$MMS_HOME"/mms_claude_*.py "$MMS_HOME"/mms_agy_*.py "$MMS_HOME"/mms_pi_*.py "$MMS_HOME"/mms_session_*.py; do' in text
