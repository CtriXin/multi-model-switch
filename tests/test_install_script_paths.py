...
def test_cleanup_removes_retired_pack_artifacts(tmp_path):
    ...
    for name in (
        ...
        assert not (home / ".claude" / "hooks" / name).exists(), name
    for cli in (".codex", ".claude"):
        assert not (home / cli / "skills" / "token-saver").is_symlink()
        assert not (home / cli / "skills" / "toon").is_symlink()
    assert not (home / ".codex" / "skills" / "ops-env-safe").exists()
    assert not (home / ".config" / "mms" / "ops-env-safe.toml").exists()
    assert not (home / ".mms" / "agent-packs").exists()


def test_cleanup_prunes_only_retired_claude_settings_entries(tmp_path):
    home = tmp_path / "home"
    home.mkdir()
    _plant_retired_pack_artifacts(home)
    ...
    assert commands == ["/usr/local/bin/mine.sh"]
    assert "SessionStart" not in settings.get("hooks", {})
    assert set(settings["mcpServers"]) == {"figma"}
    assert settings["statusLine"] == {"type": "command", "command": "mine"}

    backups = list((home / ".claude").glob("settings.json.mms-retired-backup.*"))
    assert len(backups) == 1
    original = json.loads(backups[0].read_text(encoding="utf-8"))
    assert "brainkeeper" in original["mcpServers"]
