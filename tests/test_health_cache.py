...
def _reset_cache():
    ...
    import mms_runtime.health_cache as m
...
def mock_empty_stats():
    with patch("mms_runtime.health_cache._load_speed_stats_real", return_value={}), \
         patch("mms_runtime.health_cache.get_speed_entry", return_value=None), \
         patch("mms_runtime.health_cache._persist_cache"):
        yield
...
class TestDetermineStatus:
    def test_ok_fresh_fast(self):
        from mms_runtime.health_cache import _determine_status
...
class TestLatencyBucket:
    def test_fast(self):
        from mms_runtime.health_cache import _determine_latency_bucket
...
class TestGetModelHealth:
    def test_returns_dict_with_entry(self, mock_empty_stats):
        from mms_runtime.health_cache import get_model_health
        with patch("mms_runtime.health_cache.get_speed_entry", return_value=_entry(800)):
...
class TestGetAllHealth:
    def test_empty_stats(self, mock_empty_stats):
        from mms_runtime.health_cache import get_all_health
    ...
    def test_returns_list_from_scoped_models(self):
        ...
        with patch("mms_runtime.health_cache._load_speed_stats_real", return_value=stats), \
             patch("mms_runtime.health_cache.get_speed_entry", return_value=None), \
             patch("mms_runtime.health_cache._persist_cache"):
            from mms_runtime.health_cache import get_all_health
...
class TestRefresh:
    def test_refresh_rebuilds(self, mock_empty_stats):
        from mms_runtime.health_cache import refresh_health_cache, get_all_health
        refresh_health_cache()
        assert get_all_health() == []
...
class TestCacheTTL:
    def test_cache_expired_when_empty(self):
        from mms_runtime.health_cache import _cache_expired
