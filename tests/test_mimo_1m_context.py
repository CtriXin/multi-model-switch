...
def test_mimo_1m_gateway_env_keeps_selector_in_mms_status_only(monkeypatch, tmp_path):
    ...
    assert env["ANTHROPIC_MODEL"].startswith("claude-sonnet-4-6")
    assert settings["env"]["ANTHROPIC_MODEL"].startswith("claude-sonnet-4-6")
    assert env["ANTHROPIC_MODEL"] != "mimo-v2.5-pro[1m]"
    assert settings["env"]["ANTHROPIC_MODEL"] != "mimo-v2.5-pro[1m]"
